...
public class ProductoDAO {
    ...
    public Boolean crearProducto(Producto p) {
        ...
        String sql = "INSERT INTO productos (codigo_producto, id_prod_gen, id_uni_medi, id_present, id_marca, stock, "
    ...
    public Producto leerProductoPorCodigo(String codigo) {
    ...
    public void actualizarProducto(Producto p) {
        /*String sql = "UPDATE productos SET id_prod_gen=?, id_uni_medi=?, id_present=?, id_marca=?, stock=?, stock_min=?, "
                   + "costo_base=?, porcent_margen=?, cantidad_medida=?, fecha_fabricacion=?, fecha_vencimiento=?"
                   + "WHERE codigo_producto=?";

        try (Connection con = conexion.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, p.getIdProdGen());
            ps.setInt(2, p.getIdUniMedi());
            ps.setInt(3, p.getIdPresent());
            ps.setInt(4, p.getIdMarca());
            ps.setInt(5, p.getStock());
            ps.setInt(6, p.getStockMin());
            ps.setDouble(7, p.getCostoBase());
            ps.setDouble(8, p.getPorcentMargen());
            ps.setDouble(9, p.getCantidadMedida());
            ps.setDate(10, new java.sql.Date(p.getFechaFabricacion().getTime()));
            ps.setDate(11, new java.sql.Date(p.getFechaVencimiento().getTime()));
            ps.setString(12, p.getCodigoProducto());
            ps.executeUpdate();
